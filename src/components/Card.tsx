...
const Card: React.FC<CardProps> = ({
  ...
  return (
    <div
      className={`bg-bg rounded-card relative flex w-full min-w-60 flex-col overflow-hidden border-4 p-2 ${border} ${heightClass}`}
    >
      {imageUrl && (
        <img
          src={imageUrl}
          alt={imageAlt}
          className="rounded-card mb-2 h-50 w-full shrink-0 object-cover"
        />
      )}
      <div className="font-rubik text-lg font-bold">{title}</div>
      <div className="text-base">{subtitle}</div>
      <div className="text-base">{dates}</div>
